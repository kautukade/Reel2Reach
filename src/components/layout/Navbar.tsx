  ...
  return (
    <>
      <motion.nav
        ...
        className={`fixed left-0 right-0 top-0 z-50 transition-all duration-500 ${scrolled ? 'border-b border-white/[0.06] bg-[#07090F]/78 shadow-[0_18px_45px_rgba(0,0,0,0.2)] backdrop-blur-2xl' : 'bg-transparent'}`}
      >
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between lg:h-20">
            <Link to="/" className="group flex items-center gap-2.5">
              <motion.div whileHover={reduceMotion ? undefined : { rotate: -8, scale: 1.06 }} transition={{ type: 'spring', stiffness: 350, damping: 20 }} className="relative flex h-9 w-9 items-center justify-center overflow-hidden rounded-xl border border-white/10 bg-gradient-to-br from-[#FF5A5F] via-[#FF3D8D] to-[#8B5CF6] shadow-[0_10px_28px_rgba(255,61,141,0.2)]">
              ...
              <div className="leading-none"><span className="block font-[family-name:var(--font-display)] text-base font-bold tracking-[-0.03em] text-white lg:text-lg">Reel2Reach</span><span className="mt-1 hidden text-[8px] font-semibold uppercase tracking-[0.28em] text-white/35 sm:block">Media Studio</span></div>
            </Link>
            ...
            <button onClick={() => setMobileOpen((open) => !open)} className="relative z-50 flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/[0.04] text-white backdrop-blur-xl lg:hidden" aria-label={mobileOpen ? 'Close menu' : 'Open menu'} aria-expanded={mobileOpen}>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div initial={reduceMotion ? { opacity: 0 } : { opacity: 0, clipPath: 'circle(0% at 91% 5%)' }} animate={{ opacity: 1, clipPath: 'circle(150% at 91% 5%)' }} exit={reduceMotion ? { opacity: 0 } : { opacity: 0, clipPath: 'circle(0% at 91% 5%)' }} transition={{ duration: reduceMotion ? 0.15 : 0.62, ease: [0.22, 1, 0.36, 1] }} className="fixed inset-0 z-40 overflow-y-auto bg-[#07090F]/98 backdrop-blur-2xl lg:hidden">
            ...
            <div className="relative flex min-h-full flex-col justify-center px-6 py-24">
              <p className="mb-6 text-[10px] font-semibold uppercase tracking-[0.3em] text-white/35">Navigate / Reel2Reach</p>
              <div className="space-y-0.5">
                {navLinks.map((link, index) => {
                  const active = isActive(location.pathname, link.path);
                  return <motion.div key={link.path} initial={reduceMotion ? { opacity: 0 } : { opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: reduceMotion ? 0 : 0.12 + index * 0.04, duration: 0.42 }}><Link to={link.path} className={`group flex items-center justify-between border-b border-white/[0.06] py-2.5 font-[family-name:var(--font-display)] text-2xl font-semibold tracking-[-0.04em] transition-colors sm:text-4xl ${active ? 'text-white' : 'text-white/62 hover:text-white'}`}><span>{link.label}</span><ArrowUpRight size={20} className="text-white/25 transition group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-[#FF6AA7]" /></Link></motion.div>;
                })}
              </div>
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: reduceMotion ? 0 : 0.5 }} className="mt-7 grid grid-cols-2 gap-3"><a href="https://wa.me/918263058461?text=Hi%20Reel2Reach%20Media%2C%0AI%20would%20like%20to%20discuss%20social%20media%20promotion%2Fcontent%20creation%20for%20my%20business." target="_blank" rel="noopener noreferrer" className="rounded-full border border-[#25D366]/35 bg-[#25D366]/5 px-5 py-3 text-center text-sm font-semibold text-[#8BF0A9]">WhatsApp</a><Link to="/book" className="rounded-full bg-gradient-to-r from-[#FF5A5F] to-[#FF3D8D] px-5 py-3 text-center text-sm font-semibold text-white">Book Now</Link></motion.div>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
