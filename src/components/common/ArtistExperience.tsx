...
const accentMap: Record<Accent, { glow: string; gradient: string; text: string }> = {
  pink: {
    glow: 'rgba(255,61,141,0.28)',
    gradient: 'linear-gradient(145deg,#ff5a5f 0%,#ff3d8d 48%,#241126 100%)',
    text: '#FF6AA7',
  },
  purple: {
    glow: 'rgba(139,92,246,0.28)',
    gradient: 'linear-gradient(145deg,#8b5cf6 0%,#ff3d8d 50%,#101225 100%)',
    text: '#B48AFF',
  },
  coral: {
    glow: 'rgba(255,90,95,0.28)',
    gradient: 'linear-gradient(145deg,#ff7a59 0%,#ff5a5f 45%,#23111a 100%)',
    text: '#FF8A78',
  },
};
...
export function TiltSurface({ children, className = '' }: { children: ReactNode; className?: string }) {
  ...
  return (
    <motion.div
      whileHover={reduceMotion ? undefined : { y: -8, rotateX: 2.5, rotateY: -2.5, scale: 1.012 }}
...
function ReelFace({ accent, label, title, subtitle }: { accent: Accent; label: string; title: string; subtitle: string }) {
  ...
  return (
    <div className="absolute inset-2 overflow-hidden rounded-[34px]" style={{ background: tone.gradient }}>
      ...
      <motion.div
        aria-hidden="true"
        className="absolute -left-16 top-[42%] h-44 w-44 rounded-full bg-white/10 blur-3xl"
        animate={{ x: [0, 24, 0], y: [0, -18, 0] }}
        transition={{ duration: 7, repeat: Infinity, ease: 'easeInOut' }}
      />
      <div className="absolute left-4 right-4 top-5 flex items-center justify-between">
        <span className="rounded-full border border-white/20 bg-black/15 px-3 py-1 text-[9px] font-bold tracking-[0.2em] text-white/90 backdrop-blur-xl">{label}</span>
        <div className="flex h-9 w-9 items-center justify-center rounded-full border border-white/20 bg-black/15 backdrop-blur-xl">
          <Play size={14} fill="currentColor" />
        </div>
      </div>
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-[#07090F]/95 via-[#07090F]/55 to-transparent p-5 pt-20">
        <p className="mb-2 text-[9px] font-semibold tracking-[0.24em] text-white/50">REEL2REACH / CREATIVE LAB</p>
        <h3 className="font-[family-name:var(--font-display)] text-2xl font-bold leading-[0.95]">{title}</h3>
        <p className="mt-3 text-xs leading-relaxed text-white/65">{subtitle}</p>
        <div className="mt-4 flex items-center justify-between border-t border-white/10 pt-4 text-white/60">
          <div className="flex gap-3"><Heart size={14} /><MessageCircle size={14} /><Share2 size={14} /></div>
...
export function ArtHero({
  ...
  primary,
  secondary,
  visualLabel = 'CREATIVE DIRECTION',
  visualTitle = 'MAKE THEM STOP',
  visualSubtitle = 'A cinematic social-first visual system designed for attention.',
  accent = 'pink',
}: {
  eyebrow: string;
  lines: string[];
    ...
    <section className="relative overflow-hidden pb-20 pt-28 sm:pt-32 lg:min-h-[86vh] lg:pb-24">
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-14 px-4 sm:px-6 lg:grid-cols-[1.05fr_.95fr] lg:px-8">
        ...
        <motion.div
          ...
          className="relative mx-auto h-[520px] w-full max-w-[510px] sm:h-[610px]"
          style={{ perspective: 1200 }}
        >
          <motion.div
            aria-hidden="true"
            className="absolute left-1/2 top-1/2 h-[390px] w-[390px] -translate-x-1/2 -translate-y-1/2 rounded-full blur-[100px]"
            style={{ background: tone.glow }}
            animate={reduceMotion ? undefined : { scale: [0.92, 1.08, 0.92], opacity: [0.55, 0.82, 0.55] }}
            transition={{ duration: 8, repeat: Infinity, ease: 'easeInOut' }}
          />

          <motion.div
            className="absolute left-1/2 top-1/2 h-[460px] w-[230px] -translate-x-1/2 -translate-y-1/2 sm:h-[540px] sm:w-[270px]"
            style={reduceMotion ? undefined : { rotateX, rotateY, x: cardX, y: cardY, transformStyle: 'preserve-3d' }}
          >
            <motion.div
              className="relative h-full w-full rounded-[42px] border border-white/20 bg-[#080A10] p-1 shadow-[0_42px_120px_rgba(0,0,0,0.55)]"
              animate={reduceMotion ? undefined : { y: [0, -10, 0] }}
              transition={{ duration: 5.5, repeat: Infinity, ease: 'easeInOut' }}
            >
              <div className="absolute left-1/2 top-2 z-20 h-5 w-24 -translate-x-1/2 rounded-full bg-[#07090F]" />
              <ReelFace accent={accent} label={visualLabel} title={visualTitle} subtitle={visualSubtitle} />
              <div className="pointer-events-none absolute inset-0 rounded-[42px] ring-1 ring-inset ring-white/10" />
              <motion.div
                aria-hidden="true"
                className="pointer-events-none absolute -left-10 top-[-18%] h-28 w-[150%] rotate-12 bg-gradient-to-b from-transparent via-white/16 to-transparent blur-xl"
                animate={reduceMotion ? undefined : { top: ['-22%', '120%'] }}
          ...
          <motion.div
            className="absolute left-[4%] top-[14%] w-36 rounded-2xl border border-white/12 bg-[#0B0E16]/85 p-4 shadow-2xl backdrop-blur-xl sm:w-44"
            style={reduceMotion ? undefined : { x: useTransform(smoothX, [0, 1], [-12, 12]), y: useTransform(smoothY, [0, 1], [-8, 8]) }}
            animate={reduceMotion ? undefined : { rotate: [-5, -2.5, -5] }}
            transition={{ duration: 5, repeat: Infinity, ease: 'easeInOut' }}
          >
            <p className="text-[9px] font-semibold tracking-[0.22em] text-[#FF6AA7]">HOOK / 00:03</p>
            <p className="mt-2 font-[family-name:var(--font-display)] text-sm font-bold">ATTENTION FIRST.</p>
            <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-white/8"><motion.div className="h-full bg-gradient-to-r from-[#FF5A5F] to-[#8B5CF6]" animate={reduceMotion ? undefined : { width: ['22%', '86%', '44%'] }} transition={{ duration: 6, repeat: Infinity }} /></div>
          </motion.div>

          <motion.div
            className="absolute bottom-[14%] right-[1%] w-40 rounded-2xl border border-white/12 bg-[#0B0E16]/85 p-4 shadow-2xl backdrop-blur-xl sm:w-48"
            style={reduceMotion ? undefined : { x: useTransform(smoothX, [0, 1], [11, -11]), y: useTransform(smoothY, [0, 1], [8, -8]) }}
            animate={reduceMotion ? undefined : { rotate: [4, 1.5, 4] }}
            transition={{ duration: 5.8, repeat: Infinity, ease: 'easeInOut' }}
          >
            <p className="text-[9px] font-semibold tracking-[0.22em] text-[#B48AFF]">EDIT TIMELINE</p>
            <div className="mt-3 flex gap-1.5">{[38, 72, 52, 88, 62].map((height, index) => <motion.span key={height + index} className="w-full rounded-full bg-white/16" animate={reduceMotion ? undefined : { height: [`${height * .22}px`, `${height * .36}px`, `${height * .22}px`] }} transition={{ duration: 2.4 + index * .18, repeat: Infinity }} />)}</div>
          </motion.div>
        </motion.div>
      </div>
    </section>
